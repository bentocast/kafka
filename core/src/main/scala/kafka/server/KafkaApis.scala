...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  /**
    * ADP request logger
    * Call this method from each request handler that needs to be logged
    */
  def logHeaderInfo(context: RequestContext, header: RequestHeader, topicPartitionSet: Set[String], groupId: String = "unknown"): Unit = {
    if (header != null) {
      val apiKey = header.apiKey().id
      val apiVersion = header.apiVersion()
      val clientId = header.clientId()

      ClientRequestFormatAppender.appendIntoQueue(apiKey, apiVersion, clientId, topicPartitionSet, context.connectionId, groupId)
    }
  }
  ...
  def handleOffsetCommitRequest(request: RequestChannel.Request) {
    ...
    // log request
    try {
      logHeaderInfo(request.context, request.header, offsetCommitRequest.offsetData().keySet().asScala.map(tp => tp.topic()), offsetCommitRequest.groupId())
    } catch {
      case ex: Exception => headerExtractedInfo.debug("Could not log request: Exception: " + ex.getMessage)
    }
  ...
  def handleProduceRequest(request: RequestChannel.Request) {
    ...
    // log request
    try {
      logHeaderInfo(request.context, request.header, produceRequest.partitionRecordsOrFail().keySet().asScala.map(tp => tp.topic()), "unknown")
    } catch {
      case ex: Exception => headerExtractedInfo.debug("Could not log request: Exception: " + ex.getMessage)
    }
  ...
  def handleFetchRequest(request: RequestChannel.Request) {
    ...
    // log request
    try {
      logHeaderInfo(request.context, request.header, fetchRequest.fetchData().keySet().asScala.map(tp => tp.topic()), "unknown")
    } catch {
      case ex: Exception => headerExtractedInfo.debug("Could not log request: Exception: " + ex.getMessage)
    }
  ...
  def handleTopicMetadataRequest(request: RequestChannel.Request) {
    ...
    // log request
    try {
      logHeaderInfo(request.context, request.header, topics, "unknown")
    } catch {
      case ex: Exception => headerExtractedInfo.debug("Could not log request: Exception: " + ex.getMessage)
    }
