...
object ClientRequestFormatAppender {
  ...
  val consumerHeaderInfo = new ArrayBlockingQueue[String](100000)
  ...
  def appendIntoQueue(header: RequestHeader, body: AbstractRequest, connectionId: String) = Future {
    ...
    if (api_key == ApiKeys.OFFSET_COMMIT.id) {
      ...
      val commitOffsetRecord = "{apiKey:" + api_key +
        ...
        "}"
      if(!consumerHeaderInfo.offer(commitOffsetRecord)) overflowAggregationNum.incrementAndGet()
    } else {
      val produceORFetchRecord = "{apiKey:" + api_key +
        ...
        ",topics:" + topics +
        "}"
      if(!consumerHeaderInfo.offer(produceORFetchRecord)) overflowAggregationNum.incrementAndGet()
    }
  }
}
